...
import type { CashFlowData, FinancialHealthScore, HistoricalTrends, HistoricalMonth } from '../types/analytics';
import { formatCurrency, getStoredCurrency } from '../utils/currency';
import { useDocumentTitle } from '../hooks/useDocumentTitle';

export const AnalyticsPage: React.FC = () => {
  useDocumentTitle('Analytics & Telemetry');
  ...
  const handleExportData = async () => {
    try {
      ...
      a.href = url;
      a.download = `lifetrack_telemetry_export_${new Date().toISOString().split('T')[0]}.json`;
  ...
  const maxMonthlyVal = Math.max(
    ...(trends?.history.map((m: HistoricalMonth) => Math.max(m.income, m.expense)) || [1000])
  );
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-3">
            ...
            <span className="text-xs px-2.5 py-0.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-600 dark:text-indigo-400 font-semibold">
              Synergy Engine
            </span>
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Score Card */}
        <div className="p-5 sm:p-6 bg-white dark:bg-gradient-to-br dark:from-slate-900 dark:to-indigo-950/30 border border-slate-200 dark:border-slate-800 rounded-2xl relative overflow-hidden flex flex-col justify-between shadow-xs dark:shadow-xl transition-colors">
          <div>
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                Institutional Health Metric
              </span>
              <span className="text-[10px] uppercase font-bold px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 flex items-center gap-1">
            ...
            {loading ? (
              ...
              <div className="flex items-baseline gap-2 mb-2">
                <span className="text-5xl font-extrabold font-mono text-slate-900 dark:text-white tracking-tight">
                  {healthScore?.score || 0}
                </span>
          ...
          {healthScore && (
            <div className="space-y-3 pt-4 border-t border-slate-200 dark:border-slate-800/80 text-xs">
              <div>
                <div className="flex justify-between text-[11px] mb-1">
                  <span className="text-slate-500 dark:text-slate-400">Savings Rate Score</span>
              ...
              <div>
                <div className="flex justify-between text-[11px] mb-1">
                  <span className="text-slate-500 dark:text-slate-400">Budget Adherence Score</span>
                  <span className="font-mono text-slate-800 dark:text-slate-200">
              ...
              <div>
                <div className="flex justify-between text-[11px] mb-1">
                  <span className="text-slate-500 dark:text-slate-400">Habit Discipline Synergy</span>
                  <span className="font-mono text-slate-800 dark:text-slate-200">
        ...
        <div className="lg:col-span-2 p-5 sm:p-6 bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 rounded-2xl flex flex-col justify-between shadow-xs dark:shadow-xl transition-colors">
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto -mx-2 sm:mx-0 pb-1">
              <div className="h-56 flex items-end justify-between gap-3 sm:gap-4 pt-6 px-2 border-b border-slate-200 dark:border-slate-800 min-w-[420px]">
                {trends?.history.map((m: HistoricalMonth) => {
                  ...
                  return (
                    ...
                    >
                      {/* Tooltip on hover */}
                      <div className="absolute -top-12 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-900 text-white dark:bg-slate-950 border border-slate-700 dark:border-slate-800 p-2 rounded-lg text-[10px] pointer-events-none shadow-xl z-20 whitespace-nowrap">
                      ...
                      <div className="w-full flex items-end justify-center gap-1.5 h-44">
                        {/* Income Bar */}
                        <div
                          className="w-1/2 max-w-[18px] bg-emerald-500/80 hover:bg-emerald-500 rounded-t transition-all duration-300"
                          style={{ height: `${Math.max(4, incomePct)}%` }}
                        />
                        {/* Expense Bar */}
                        <div
                          className="w-1/2 max-w-[18px] bg-rose-500/80 hover:bg-rose-500 rounded-t transition-all duration-300"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-4 text-xs text-slate-500 dark:text-slate-400">
            <span className="flex items-center gap-1">
              <Info className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500 shrink-0" />
              Hover or tap columns for detailed month-over-month ledger totals
            </span>
