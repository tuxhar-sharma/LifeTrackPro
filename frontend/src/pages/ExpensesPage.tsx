...
export const ExpensesPage: React.FC = () => {
  ...
  const totalSpent = expenses.reduce((sum, e) => sum + (e.amount_cents / 100), 0);
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5 sm:gap-3">
          ...
          <div className="flex items-center gap-2 bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-1.5 shadow-xs">
            <Coins className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400 shrink-0" />
            <span className="text-[11px] uppercase font-semibold text-slate-500 dark:text-slate-400">Currency:</span>
            <select
              value={currentCurrency}
              onChange={(e) => handleCurrencyChange(e.target.value)}
              className="bg-transparent font-bold text-xs text-slate-900 dark:text-white focus:outline-none cursor-pointer pr-1"
            >
              {CURRENCIES.map((c) => (
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-xs font-medium text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 transition-all shadow-xs cursor-pointer"
          >
      ...
      {budgets.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {budgets.map((b) => (
            <div
              key={b.id}
              className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-xs dark:shadow-lg space-y-2.5 transition-colors"
            >
              <div className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className="font-semibold text-slate-900 dark:text-white">{b.category_name}</span>
                </div>
              ...
              <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                <span>
                  Spent: <b className="text-slate-900 dark:text-white">{formatCurrency(b.spent_display, currentCurrency)}</b>
                </span>
                <span>
                  Limit: <b className="text-slate-900 dark:text-white">{formatCurrency(b.limit_display, currentCurrency)}</b>
                </span>
              </div>
            </div>
          ))}
      ...
      <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 shadow-xs transition-colors">
        <form onSubmit={handleSearchSubmit} className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-all"
          />
        </form>
        ...
        <div className="flex flex-wrap items-center justify-between sm:justify-end gap-3 w-full sm:w-auto">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-700 dark:text-slate-300 px-3 py-2 focus:outline-none focus:border-indigo-500 cursor-pointer"
            >
          ...
          <div className="pl-3 border-l border-slate-200 dark:border-slate-800 text-xs text-slate-500 dark:text-slate-400">
            Filtered Total: <b className="text-slate-900 dark:text-white">{formatCurrency(totalSpent, currentCurrency)}</b>
          </div>
        </div>
      </div>

      {/* Ledger Table */}
      <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-xs dark:shadow-xl transition-colors">
        <div className="overflow-x-auto -mx-2 sm:mx-0">
          <table className="w-full text-left text-xs min-w-[650px]">
            <thead className="bg-slate-50 dark:bg-slate-950/40 text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold border-b border-slate-200 dark:border-slate-800">
              <tr>
              ...
              {loading ? (
                ...
                expenses.map((exp) => (
                  <tr key={exp.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-slate-500 dark:text-slate-400">{exp.transaction_date}</td>
                    <td className="py-3.5 px-4 capitalize text-slate-500 dark:text-slate-400">
        ...
        <form onSubmit={handleCreateExpense} className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="col-span-1">
              ...
              <select
                ...
                className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 cursor-pointer font-medium"
              >
                {CURRENCIES.map((c) => (
                  <option key={c.code} value={c.code} className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">
                    {c.code} ({c.symbol})
            ...
            <div className="col-span-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="45.50"
                  className="w-full pl-8 pr-3 py-2 bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-xl text-sm text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500"
                />
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            <button
              ...
              className="px-4 py-2 rounded-xl text-xs font-medium text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors cursor-pointer"
            >
              Cancel
            </button>
            <button
        ...
        <form onSubmit={handleCreateBudget} className="space-y-4">
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold shadow-lg shadow-emerald-600/30 transition-all cursor-pointer disabled:opacity-50"
            >
