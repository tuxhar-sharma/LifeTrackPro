    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-xs text-slate-500 dark:text-slate-400">Loading Telemetry Overview...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5 sm:gap-3">
          <Link
            to="/app/expenses"
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-xs font-medium text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 transition-all shadow-xs"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 sm:p-6 shadow-xs dark:shadow-xl transition-colors">
                ...
                <div
                  ...
                  className={`flex items-center justify-between p-3.5 rounded-xl border transition-all cursor-pointer ${
                    habit.is_completed_today
                      ? 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-500/30 text-slate-800 dark:text-slate-200'
                      : 'bg-slate-50/70 dark:bg-slate-950/40 border-slate-200 dark:border-slate-800/80 text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800/40'
                  }`}
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <button
                      ...
                      className="text-slate-400 hover:text-emerald-500 dark:hover:text-emerald-400 transition-colors shrink-0"
                    >
                      {habit.is_completed_today ? (
                    ...
                    <div className="min-w-0">
                      <span
                        className={`text-sm font-medium truncate block ${
                          habit.is_completed_today ? 'line-through text-slate-400 dark:text-slate-500' : 'text-slate-900 dark:text-white'
                        }`}
                      >
                        {habit.name}
                      </span>
                  ...
                  <div className="flex items-center gap-2 shrink-0 ml-2">
                    <span className="flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20">
        ...
        <div className="lg:col-span-5 bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 sm:p-6 shadow-xs dark:shadow-xl flex flex-col justify-between transition-colors">
          <div>
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2.5">
                <div className="p-2 bg-indigo-500/10 rounded-lg text-indigo-600 dark:text-indigo-400">
          ...
          <div className="mt-6 pt-4 border-t border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
            ...
            <span className="font-bold text-slate-900 dark:text-white">{formatCurrency(totalSpent, getStoredCurrency())}</span>
          </div>
        </div>
      </div>

      {/* Recent Ledger Stream */}
      <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 sm:p-6 shadow-xs dark:shadow-xl transition-colors">
        ...
        <div className="overflow-x-auto -mx-2 sm:mx-0">
          <table className="w-full text-left text-xs min-w-[500px]">
            <thead className="text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800 uppercase tracking-wider font-semibold">
              <tr>
              ...
              {(!summary?.recent_expenses || summary.recent_expenses.length === 0) ? (
                ...
                summary.recent_expenses.map((exp) => (
                  <tr key={exp.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="py-3 px-3 text-slate-500 dark:text-slate-400">{exp.transaction_date}</td>
                    <td className="py-3 px-3 text-slate-500 dark:text-slate-400 capitalize">{exp.payment_method.replace('_', ' ')}</td>
